...
This document covers the general-purpose engine in this `v2/` folder
(everything except `nasa_procurement/`). For the flagship real-data
implementation — live USASpending.gov API pulls, UEI/DUNS-based supplier
resolution, a three-agent workflow, and obligation/deobligation-aware
analytics — see `nasa_procurement/PROJECT_SUMMARY.md` instead. See the
root-level pitch for how the two relate.
...
61 tests, all offline/deterministic (the LLM fallback tests mock the
