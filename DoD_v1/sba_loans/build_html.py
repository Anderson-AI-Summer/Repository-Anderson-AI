...
with open(r"C:\finances\data\sba_loans\_data.json") as f:
    DATA = json.load(f)

TOTAL = DATA["total"]
ROWS = DATA["rows"]

with open(r"C:\finances\data\sba_loans\_vendor_tab_data.json") as f:
...
with open(r"C:\finances\data\sba_loans\_monthly_final.json") as f:
    MONTHLY = json.load(f)
...
with open(r"C:\finances\data\sba_loans\_costtype_final.json") as f:
    COSTTYPE_ROWS = json.load(f)
COST_TYPE_TOTAL = 145237923820.19
FIXED_PRICE_TOTAL = 342221714944.87
CONTRACT_TOTAL_ALL = 491670911929.61
...
with open(r"C:\finances\data\sba_loans\_singlebid_final.json") as f:
...
with open(r"C:\finances\data\sba_loans\_drilldown_final.json") as f:
...
html = f"""<title>DoD FY2025 Spend Taxonomy</title>
<style>
@font-face {{
...
<div class="page">
  <header class="masthead">
    ...
    <div class="masthead-row">
      <div>
        <h1>Department of Defense &mdash; FY2025 Spend Analysis</h1>
        <p class="dek">Fiscal Year 2025 obligations (Oct 2024&ndash;Sep 2025): what DoD buys, grouped into 18 spend categories, and who it buys from, with a screen for contracts classified under vague catch-all industry codes.</p>
      </div>
  ...
  <section class="tabpanel is-active" id="panel-taxonomy" role="tabpanel" aria-labelledby="tab-btn-taxonomy">
    <section class="kpi-row">
      ...
      <div class="kpi">
        ...
        <span class="kpi-value">20.8%</span>
        <span class="kpi-sub">Aircraft &amp; Aviation Systems</span>
      </div>
      <div class="kpi">
        <span class="kpi-label">NAICS Coverage</span>
        <span class="kpi-value">92.3%</span>
        <span class="kpi-sub">of total, from top-100 codes</span>
      </div>
      <div class="kpi">
        <span class="kpi-label">Contract Share</span>
        <span class="kpi-value">98.0%</span>
    ...
    <footer class="methodology">
      ...
      <ul>
        ...
        <li>Scope: Department of Defense as awarding agency, all award types, action dates 2024-10-01 through 2025-09-30 (FY2025).</li>
        <li>The top 100 NAICS industry codes by dollar amount were retrieved (92.3% of total obligations) and grouped by hand into 18 mission-relevant categories; the remainder &mdash; the long tail of smaller NAICS codes plus grants, direct payments, and other non-contract assistance &mdash; is shown as <strong>Other</strong>.</li>
        ...
        <li>DoD contract awards carry a 90-day publication delay under federal disclosure rules, so the most recent weeks of FY2025 activity may be slightly underrepresented.</li>
      </ul>
    </footer>
  </section>
  ...
  <section class="tabpanel" id="panel-vendors" role="tabpanel" aria-labelledby="tab-btn-vendors">
    <div class="callout">
      ...
      <div class="callout-body">
        <p><strong>What this is:</strong> the top 30 DoD vendors by FY2025 dollar value, cross-referenced against how much of each vendor's spend falls under Census Bureau <strong>catch-all NAICS codes</strong> &mdash; industry classifications literally titled "All Other&hellip;" that get used when a business's work doesn't fit a more specific category.</p>
    ...
    <footer class="methodology">
      <h3>Methodology &amp; Caveats</h3>
      <ul>
        <li>Source: USAspending.gov public award API, queried live. Vendors are ranked by total DoD-awarded dollar value for FY2025 (2024-10-01 through 2025-09-30), consolidated across registered entity-name variants (e.g. "Lockheed Martin Corp" and "Lockheed Martin Corporation" are combined).</li>
        ...
        <li>DoD contract awards carry a 90-day publication delay, so the most recent weeks of FY2025 activity may be underrepresented for all vendors.</li>
      </ul>
    </footer>
  </section>
  ...
  <section class="tabpanel" id="panel-patterns" role="tabpanel" aria-labelledby="tab-btn-patterns">
    ...
    <section class="panel">
      ...
      <div class="finding" style="margin-bottom:18px;">
        <strong>Leland Limited Inc &mdash; Defense Logistics Agency (Navy/Marine supply) &mdash; 7 single-bid awards, same product, near-monthly cadence.</strong> Every one of Leland's FY2024&ndash;25 contracts with DLA was for <em>Marine Lifesaving and Diving Equipment</em>, ran $224K&ndash;$240K, was nominally competed under Simplified Acquisition Procedures, and drew exactly one bidder &mdash; Leland. That's consistent with a genuinely thin supplier market for specialized diving/marine-safety gear (in which case one bidder every time is simply reality, not a process failure) &mdash; but it's also exactly the pattern worth a contracting officer double-checking: are other qualified suppliers being notified of these solicitations, or is the sourcing list effectively down to one company by default?
    ...
    <section class="panel">
      <div class="panel-head">
        <h2>Obligation Timing Across FY2025</h2>
      </div>
      <p class="dek" style="margin-bottom:16px;">Common government-spending folklore says agencies rush to obligate before the fiscal year ends in September ("use it or lose it"). That's not what DoD's FY2025 data shows.</p>
      <div class="kpi-row">
        <div class="kpi">
          <span class="kpi-label">Peak Month</span>
          <span class="kpi-value">{PEAK_MONTH['month']} '24</span>
      ...
      <div class="finding" style="margin-top:16px;">
        <strong>December 2024 obligated {PEAK_MONTH['total']/MONTHLY_AVG:.1f}× the monthly average</strong> — by far the largest month in FY2025, dwarfing September's fiscal year-end ({SEP_ROW['vs_avg_pct']:+.0f}% vs. average, i.e. essentially a normal month). The most common driver of a pattern like this is appropriations timing: DoD frequently operates under a Continuing Resolution at the start of the fiscal year, which restricts new-start spending until a full-year appropriations bill (often passed in December) unlocks it — producing a backlog release rather than a deliberate year-end rush. December and March also carried the highest non-competitive shares (53.9% and 60.9% of dollars, vs. ~40–52% in other months), consistent with obligations moving faster, with less time for competitive procedures, right after funding unlocks. This is a plausible explanation, not a proven cause — we have not traced individual December awards back to specific appropriations actions.</div>
    </section>
    ...
    <section class="panel">
      ...
      <div class="costtype-legend-row">
        ...
        <div class="costtype-stat"><b>{FULL_COST_TYPE_VENDORS}</b>&nbsp;of the top 30 vendors are 100% cost-type</div>
      </div>
      <div class="bars" id="ctbars"></div>
      <div class="finding" style="margin-top:16px;">
        The five vendors at 100% cost-type are not a red flag — they're exactly who you'd expect: <strong>Humana Government Business</strong> and <strong>TriWest Healthcare Alliance</strong> administer TRICARE health plans (cost-reimbursement by design), <strong>Bechtel Plant Machinery</strong> and <strong>Fluor Marine Propulsion</strong> sustain naval nuclear reactors (unique, sole-source expertise), and <strong>Johns Hopkins APL</strong> is a federally funded R&amp;D center (FFRDCs are structured as cost-type). Large primes cluster in the 30–75% range, reflecting their mix of production (fixed-price) and R&amp;D/sustainment (cost-type) work. Nothing here indicates impropriety on its own — it's a map of where cost-growth risk sits, useful alongside other signals, not instead of them.
      </div>
    </section>
    ...
    <section class="panel">
      ...
      <div class="finding null-result">
        <strong>Initial pattern looked significant, but didn't hold up.</strong> We found {BUNCH_TOTAL_CLUSTERS} same-vendor/same-day/same-agency clusters across 8 candidate vendors, totaling {fmt_b(BUNCH_TOTAL_STANDALONE_DOLLARS)}, even after excluding clusters that shared one existing parent supply vehicle (i.e. routine multiple calls under an already-competed contract, which is normal). But the two vendors driving nearly all of it — <strong>AmerisourceBergen Drug Corp</strong> (120 clusters, {fmt_b(121717717.41)}) and <strong>Atlantic Diving Supply</strong> (21 clusters, {fmt_b(22382773.27)}) — are large, established DLA distributors (pharmaceuticals and tactical/diving gear) who hold <em>multiple concurrent</em> supply IDVs as their normal operating model. Getting several separate calls from DLA on the same day, each against a different existing vehicle, is exactly what a high-volume distributor's business looks like — not evidence of splitting one requirement into two to dodge competition.<br><br>
        A genuinely reliable test would need to compare the <em>item-level descriptions</em> of same-day awards to check whether they're really the same requirement split in two — that data isn't available through USAspending's bulk API. We're reporting this as a tested-and-inconclusive result rather than either hiding it or overstating it.
      </div>
    </section>
    ...
    <footer class="methodology">
      <h3>Methodology &amp; Caveats</h3>
      <ul>
        <li>Source: USAspending.gov public award API, queried live. Scope: Department of Defense, FY2025 (2024-10-01 through 2025-09-30), contract award types only.</li>
        <li><strong>Obligation timing</strong> uses actual (calendar) obligation month via each award's action date; "non-competitive" uses the same B/C/G extent-competed definition as the Vendor Concentration tab.</li>
        <li><strong>Cost-type mix</strong> uses FPDS contract pricing type codes: cost-reimbursement = Cost Plus Award Fee, Cost No Fee, Cost Sharing, Cost Plus Fixed Fee, Cost Plus Incentive Fee; fixed-price = Firm Fixed Price and its variants. Time-and-materials contracts (0.3% of dollars) are excluded from both buckets.</li>
        <li><strong>Threshold clustering</strong> was tested on the 8 vendors with the highest dollar concentration in the $300,000–$349,999 band; "same parent vehicle" is inferred from each award's underlying IDV/BPA reference number.</li>
        <li><strong>Single-bid competed contracts</strong> covers all 17,701 DoD definitive contracts (award type "D") for FY2025, pulled via USAspending's bulk award-download endpoint (the only place "Number of Offers Received" is exposed &mdash; it's absent from the standard search API). "Genuinely competed" = Full and Open Competition, Full and Open Competition after Exclusion of Sources, or Competed under Simplified Acquisition Procedures; sole-source categories are excluded since a single offer is expected by design for those. <strong>Branch</strong> in the drill-down table is the awarding sub-agency reported on the award (Army, Navy, Air Force, Defense Logistics Agency, and other DoD components) &mdash; not necessarily where the equipment or service is ultimately used.</li>
...
document.getElementById('cdDownloadBtn').addEventListener('click', () => {{
  ...
  a.href = url;
  a.download = 'dod_singlebid_contracts_under_350k.csv';
...
with open(r"C:\finances\data\sba_loans\dod_fy25_taxonomy_dashboard.html", "w", encoding="utf-8") as f:
    f.write(html)
