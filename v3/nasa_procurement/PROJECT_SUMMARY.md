...
`outputs/nasa_live_dashboard.html` (Harrison) adds a second dashboard mode
that queries `api.usaspending.gov` directly from the browser for any fiscal
year (FY2008-present) -- no server, no embedded data, always current.
Deliberately kept as a separate, cross-linked page rather than merged into
the main dashboard: the analysis tabs above depend on this project's own
Python pipeline (supplier resolution, taxonomy classification, HHI,
standout/consolidation/duplicate signals), none of which exist as a live
endpoint to call, so Live Lookup shows raw, unnormalized API figures and
