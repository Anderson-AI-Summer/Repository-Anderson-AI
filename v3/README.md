...
Third milestone. Copies `v2/` forward unchanged and adds one new capability
on top, in two tiers:

1. **[`nasa_procurement/`](nasa_procurement/)** — the flagship real-data
   implementation (Lindsay), now with a second mode (Harrison): alongside
   the precomputed analysis dashboard, `outputs/nasa_live_dashboard.html`
   queries `api.usaspending.gov` directly from the browser for any fiscal
   year back to FY2008 -- no server, no stored data, always current. Pulls
   NASA prime-contract transactions from the USAspending.gov API
   (FY2020–present for the analysis pipeline), resolves supplier identity on
   UEI/DUNS evidence (not just name matching), classifies spend on a
   NASA-specific PSC/NAICS taxonomy, derives signed/deobligation-aware
   analytics (HHI concentration, tail spend, YoY category shifts), and
   narrates findings through a three-agent workflow (deterministic code owns
   all arithmetic; agents only adjudicate ambiguity or narrate results
   already computed) into one self-contained interactive dashboard. 61 tests,
   all offline/deterministic. See `nasa_procurement/README.md` for how the
   two dashboard modes relate and why they're kept separate rather than merged.
2. **The general engine below** (everything else in this folder) — the
   domain-agnostic core: a reusable ingest → vendor-resolution → taxonomy →
   preferred-supplier pipeline, proven across four datasets (a generic
   corporate sample, SBA PPP loans with a peer-reviewed statistical anomaly
   analysis, and two USASpending.gov demos scoped to NASA and HUD), plus a
   multi-dataset comparison dashboard and an optional LLM classification
   fallback. This is what the rest of this README documents.

Both tiers independently fulfill the assignment brief (messy transaction
file → spend taxonomy → vendor-alias resolution → preferred-supplier bypass
flag, sourced from usaspending.gov); `nasa_procurement/` is the stronger,
more complete real-world proof, and the general engine is the more reusable,
tested-across-domains one. See the root-level pitch for the full comparison
and recommendation.

> All commands below (except `nasa_procurement/`, which has its own
> `cd nasa_procurement` + its own README) are run from inside this `v2/`
> folder (`cd v2` first). See the [repo root README](../README.md) for how
> versioning across the team works.
...
A second demo scoped to HUD instead of NASA lives alongside it
(`usaspending/data/sample_hud_contracts.csv` /
`sample_hud_preferred_suppliers.json`) — same command, different `--input`
and `--suppliers`. `dashboard/generate_visualizer.py` renders both of these
plus the generic corporate sample into one page with bar charts and a
dataset switcher (`dashboard/spend_visualizer.html`):
...
`api.usaspending.gov`, so both `usaspending/data/sample_nasa_contracts.csv`
and `sample_hud_contracts.csv` were **not** fetched from the live API —
they're hand-built, clearly-labeled illustrative samples using fictional
contractor names (e.g. "Meridian Defense Systems", "Northgate IT Solutions")
built to exercise every feature of the pipeline: one recipient under 4 raw
name variants, ten spend categories, and awards routed away from a
documented (equally fictional) preferred-supplier policy. None of it should
be read as, or confused with, a real award record — consistent with this
repo's stance elsewhere (see the PPP section below) of never attaching a
real-data-shaped claim to an entity without real data behind it.
`spend_agent/usaspending_adapter.py` itself is written against
USASpending.gov's real, documented column schema, so it works unmodified
against an actual bulk download or API export — only the bundled demo files
are synthetic. (For real NASA award data, see `nasa_procurement/` instead —
it pulls from the live API and also has a checked-in 21,240-row real FY2025
NASA CSV at `../data/nasa_fy2025_contract_transactions.csv` it can ingest
directly.)
...
misrepresent the output as a compliance finding. The `sample_*_preferred_suppliers.json`
files are fictional policies used only for the sample runs above; don't
