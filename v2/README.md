...
Second milestone. Consolidates the work everyone shipped since the `v1`
restructure into one place, in two tiers:

1. **[`nasa_procurement/`](nasa_procurement/)** — the flagship real-data
   implementation (Lindsay). Pulls NASA prime-contract transactions live from
   the USAspending.gov API (FY2020–present), resolves supplier identity on
   ...
   already computed) into one self-contained interactive dashboard. 59 tests,
   all offline/deterministic. See `nasa_procurement/README.md`.
